...
if [ "$#" == 1 ] && [ $1 == "plot" ]; then
   ...
   for file in *.fil; do
      ...
      single_pulse_search.py --maxwidth 70 --nobadblocks --detrendlen 8 test_single_dm${DM}_width${width}.dat | grep "Found"
   ...
   for i in $(seq 0 1 $dm_range); do
      ...
      echo "$row2" >> "${model}_output/${model}_boxcar.txt"
   done
fi


python3 ../graph.py injected_snr.txt ${model}_output/${model}.txt $DM_start $DM_end $DM_step $width_start $width_end $width_step ${model}
python3 ../python/boxcar.py ${model}_output/${model}_boxcar.txt $DM_start $DM_end $DM_step $width_start $width_end $width_step
